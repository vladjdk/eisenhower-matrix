...
export const COLUMNS = ['id', 'title', 'quadrant', 'done', 'due', 'notes', 'sources', 'links', 'x', 'y', 'created_at', 'aged_from', 'done_at', 'updated_at'];
...
function rowToTask(r) {
  ...
  return {
    ...
    aged_from: r.aged_from ?? '',
    done_at: r.done_at ?? '',
    updated_at: r.updated_at ?? '',
  };
}
const taskToRow = ({ generated, ...t }) => [t.id, t.title, QUADRANTS[t.q], t.done ? 'true' : 'false', t.due, t.notes, JSON.stringify(t.sources ?? []), JSON.stringify(t.links ?? []), Math.round(t.x), Math.round(t.y), t.created_at, t.aged_from ?? '', t.done_at ?? '', t.updated_at];
...
export function createStore(file) {
  ...
  return {
    ...
    upsert: t => mutate(tasks => {
      ...
      if (i < 0) { tasks.push({ ...t, sources: t.sources ?? [], links: t.links ?? [], created_at: now, aged_from: t.aged_from ?? '', done_at: t.done ? now : '', updated_at: now }); return; }
      const prev = tasks[i];
      tasks[i] = { ...prev, ...t, sources: t.sources ?? prev.sources, links: t.links ?? prev.links, aged_from: t.aged_from ?? prev.aged_from, done_at: !t.done ? '' : prev.done ? prev.done_at : now, updated_at: now };
    }),
